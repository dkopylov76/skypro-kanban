import cardsList from "/src/data.js";

const CardsItemOrange = ({id, theme, title, date}) => {
    // for (let i = 0; i < cardsList.length; ++i) {
    //     return renderCards();
    // }
        // console.log(cardList[i]);
    // cardList.forEach(({ id, theme, title, date, status }) => {
        // if (cardList[0].theme === "Web Design") {
        
        const themeMap = {"Web Design":"orange", "Research":"green", "Copywriting":"purple"};
        const themeClass = themeMap[theme]
                
        return (
        <>
            <div class="cards__item">
                <div class="cards__card card">
                    <div>{id}</div>
                    <div class="card__group">
                        <div class={`card__theme _${themeClass}`}>
                            <p class={`_${themeClass}`}> {theme} </p>
                        </div>
                        <a href="#popBrowse" target="_self">
                            <div class="card__btn">
                                <div></div>
                                <div></div>
                                <div></div>
                            </div>
                        </a>
                    </div>
                    <div class="card__content">
                        <a href="" target="_blank">
                            <h3 class="card__title"> {title} </h3>
                        </a>
                        <div class="card__date">
                            <svg xmlns="http://www.w3.org/2000/svg" width="13" height="13" viewBox="0 0 13 13" fill="none">
                                <g clipPath="url(#clip0_1_415)">
                                    <path d="M10.5625 2.03125H2.4375C1.7644 2.03125 1.21875 2.5769 1.21875 3.25V10.5625C1.21875 11.2356 1.7644 11.7812 2.4375 11.7812H10.5625C11.2356 11.7812 11.7812 11.2356 11.7812 10.5625V3.25C11.7812 2.5769 11.2356 2.03125 10.5625 2.03125Z" stroke="#94A6BE" strokeWidth="0.8" strokeLinejoin="round" />
                                    <path d="M11.7812 4.0625H1.21875M3.25 1.21875V2.03125V1.21875ZM9.75 1.21875V2.03125V1.21875Z" stroke="#94A6BE" strokeWidth="0.8" strokeLinecap="round" strokeLinejoin="round" />
                                </g>
                                <defs>
                                    <clipPath id="clip0_1_415">
                                        <rect width="13" height="13" fill="white" />
                                    </clipPath>
                                </defs>
                            </svg>
                            <p> {date} </p>
                            {/* <p> {cardsList[i].status} </p> */}
                        </div>
                    </div>
                </div>
            </div>
        </>
        )
    }
// )
// }

export default CardsItemOrange




// const CardsItemOrange = () => {
//     for (let i = 0; i < cardsList.length; ++i) {
//         // console.log(cardList[i]);
//     // cardList.forEach(({ id, theme, title, date, status }) => {
//         // if (cardList[0].theme === "Web Design") {
//         return (
//         <>
//             <div class="cards__item">
//                 <div class="cards__card card">
//                     <div> {cardsList[i].id} </div>
//                     <div class="card__group">
//                         <div class="card__theme _orange">
//                             <p class="_orange"> {cardsList[i].theme} </p>
//                         </div>
//                         <a href="#popBrowse" target="_self">
//                             <div class="card__btn">
//                                 <div></div>
//                                 <div></div>
//                                 <div></div>
//                             </div>
//                         </a>
//                     </div>
//                     <div class="card__content">
//                         <a href="" target="_blank">
//                             <h3 class="card__title"> {cardsList[i].title} </h3>
//                         </a>
//                         <div class="card__date">
//                             <svg xmlns="http://www.w3.org/2000/svg" width="13" height="13" viewBox="0 0 13 13" fill="none">
//                                 <g clipPath="url(#clip0_1_415)">
//                                     <path d="M10.5625 2.03125H2.4375C1.7644 2.03125 1.21875 2.5769 1.21875 3.25V10.5625C1.21875 11.2356 1.7644 11.7812 2.4375 11.7812H10.5625C11.2356 11.7812 11.7812 11.2356 11.7812 10.5625V3.25C11.7812 2.5769 11.2356 2.03125 10.5625 2.03125Z" stroke="#94A6BE" strokeWidth="0.8" strokeLinejoin="round" />
//                                     <path d="M11.7812 4.0625H1.21875M3.25 1.21875V2.03125V1.21875ZM9.75 1.21875V2.03125V1.21875Z" stroke="#94A6BE" strokeWidth="0.8" strokeLinecap="round" strokeLinejoin="round" />
//                                 </g>
//                                 <defs>
//                                     <clipPath id="clip0_1_415">
//                                         <rect width="13" height="13" fill="white" />
//                                     </clipPath>
//                                 </defs>
//                             </svg>
//                             <p> {cardsList[i].date} </p>
//                             <p> {cardsList[i].status} </p>
//                         </div>
//                     </div>
//                 </div>
//             </div>
//         </>
//         )
//     }
// // )
// }

// export default CardsItemOrange